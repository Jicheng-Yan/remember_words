...
import sys
import argparse
from remembering_words.deck import Deck, DeckManager
from remembering_words.session import Session
...
def interactive_mode():
    """Main interactive application function."""
    # Initialize the deck manager
    deck_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "decks")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="English Word Memorization Application"
    )
    parser.add_argument(
        "-i", "--interactive",
        action="store_true",
        help="Run the application in interactive mode (default)"
    )
    parser.add_argument(
        "--import-csv",
        metavar="CSV_FILE",
        help="Import a deck from a CSV file"
    )
    parser.add_argument(
        "--deck-name",
        metavar="NAME",
        help="Specify a deck name for importing or starting a session"
    )
    parser.add_argument(
        "--start-session",
        action="store_true",
        help="Start a session with the specified deck"
    )
    parser.add_argument(
        "--num-cards",
        type=int,
        help="Number of cards to study in a session"
    )
    parser.add_argument(
        "--list-decks",
        action="store_true",
...
def main():
    ...
    app_data_dir = os.path.join(data_dir, 'remembering_words')
    deck_dir = os.path.join(app_data_dir, 'decks')
    ...
    if args.import_csv:
        if not args.deck_name:
            print("Error: --deck-name is required when importing a CSV file.")
            return
            
        try:
            deck = deck_manager.import_deck_from_csv(args.import_csv, args.deck_name)
            print(f"Deck '{args.deck_name}' imported successfully with {len(deck.words)} words.")
        except (ValueError, FileNotFoundError) as e:
            print(f"Error: {e}")
        return
    ...
    if args.start_session:
        if not args.deck_name:
            print("Error: --deck-name is required when starting a session.")
            return
            
        try:
            deck = deck_manager.load_deck(args.deck_name)
            
            if not deck.words:
                print(f"Deck '{args.deck_name}' is empty. Add words before starting a session.")
                return
                
            num_cards = args.num_cards or len(deck.words)
            num_cards = min(num_cards, len(deck.words))
            ...
            print(f"Error: Deck '{args.deck_name}' not found.")
        return
